...
async def create_task_for_project(
    ...
    Authentication is introduced in Day 4, so `created_by` is temporarily supplied in the body.
    """
    ...
    creator = await user_crud.get_user_by_id(db, task_in.created_by)
    if creator is None:
    ...
    task = await task_crud.create_task(db, project_id, task_in)
